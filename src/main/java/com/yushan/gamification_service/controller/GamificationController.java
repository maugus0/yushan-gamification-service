...
public class GamificationController {
    ...
    public ApiResponse<UserLevelDTO> getUserLevel(@PathVariable UUID userId) {
        UserLevelDTO userLevel = gamificationService.getUserLevel(userId);
        return ApiResponse.success("User level retrieved successfully", userLevel);
    }
    ...
    public ApiResponse<String> rewardComment(@PathVariable Long commentId) {
        UUID userId = SecurityUtils.getCurrentUserId();
        gamificationService.rewardComment(userId, commentId);
        return ApiResponse.success("Comment rewarded successfully");
    }
    ...
    public ApiResponse<String> rewardReview(@PathVariable Long reviewId) {
        UUID userId = SecurityUtils.getCurrentUserId();
        gamificationService.rewardReview(userId, reviewId);
        return ApiResponse.success("Review rewarded successfully");
    }

    // Vote APIs
    @Operation(summary = "[USER] Process vote reward", description = "Award 3 EXP to user for voting")
    @PostMapping("/votes/reward")
    public ApiResponse<String> rewardVote() {
        UUID userId = SecurityUtils.getCurrentUserId();
        gamificationService.rewardVote(userId);
        return ApiResponse.success("Vote processed successfully");
    }
    ...
    public ApiResponse<VoteCheckResponseDTO> checkVoteEligibility() {
        ...
        return ApiResponse.success("Vote eligibility checked", response);
    }
    ...
    public ApiResponse<String> updateYuanAfterVote() {
        UUID userId = SecurityUtils.getCurrentUserId();
        gamificationService.updateYuanAfterVote(userId);
        return ApiResponse.success("Yuan deducted for vote");
    }
}
